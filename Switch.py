...
class RotatingZoomingRectanglesWithCircuit(Scene):
     def construct(self):
         ...
         red_rect = Polygon([0, 2, 0], [3.5, 2, 0], [3.5, -2, 0], [0, -2, 0], color=RED, fill_opacity=0.5)
         ...
         # Fade out text
         self.play(FadeOut(n_emitter_text, p_base_text, n_collector_text, bjt_text, run_time=1))
         ...
         rb_resistor = VMobject()
         rb_resistor.set_points_as_corners(rb_vertices)
         rb_resistor.set_color(YELLOW)

         # Label "Rb" above resistor
         rb_text = Text("Rb", color=YELLOW).scale(0.6).next_to(rb_resistor, UP, buff=0.2)

         # Animate resistor creation
         self.play(Create(rb_resistor, run_time=2), Write(rb_text, run_time=1))

         # Resistor (Rl) as a zig-zag line
         rl_vertices = [
         [3.5, 3.7, 0], 
         [3.7, 3.9, 0], 
         [3.9, 3.5, 0], 
         ...
         rl_resistor = VMobject()
         rl_resistor.set_points_as_corners(rl_vertices)
         rl_resistor.set_color(YELLOW)

         # Label "Rb" above resistor
         rl_text = Text("Rc", color=YELLOW).scale(0.6).next_to(rl_resistor, DOWN, buff=0.2)

         # Animate resistor creation
         self.play(Create(rl_resistor, run_time=2), Write(rl_text, run_time=1))
